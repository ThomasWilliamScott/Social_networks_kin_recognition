% INFINITE ISLAND MODEL, WEAK SELECTION (SUPP. INFO 3 & MAIN TEXT).

% This script contains the code used to generate the data saved as 
% 'weak_sel_alpha=...'. The script iterates the population genetic 
% model described in the main text and in Supp. Info. 3 (weak selection 
% island model). Parameter values must be specified as inputs. Note that,
% except where specified, all data was generated using the following
% inputs: tagSkewIni = 0.9; helpini = 0.1; b=0.3; c=0.1; T = 100000; 
% mR=[0:0.02:1]; rR=[0:0.025:0.5]; tag = 100; mu = 0.001; N=30. We record 
% two summary statistics as outputs: population cooperator freqeuncy at 
% equilibrium ('resalt'), and average tag frequency at equilibrium 
% ('avgtagfreq'; the inverse of this, 1/avgtagfreq, gives the number of 
% tags maintained at equilibrium).

clearvars
clc

% PARAMETERS TO BE SET AS INPUTS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tagSkewIni = 0.9; % This parameter captures the initial tag frequency distribution. It ranges from zero to one. When 0, all tag groups are roughly equal frequency initially. When 1, one tag starts at fixation. When intermediate, the dominant tag has some intermediate initial skew.
helpini = 0.1; % Proportion altruists within tag groups at start of run.
...
N=30; % Deme size
csearch =  0; % Cost of parter search
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% G (below) is the probability that 1 triplet of genes across 3 individuals 
% coalesces.

G = (((1-m)^3)*(1+3*F*(N-1)) ) / ( N^2 - ((1-m)^3) *(N-1)*(N-2) ) ;

for t=1:T

% selection

xi = sum(pop(:,1,:,t) ); % This array gives the population frequency of each tag
xj = sum(xi.^2) - xi.^2; % This array gives the average tag frequency amongst all individuals who lack tag i.
pi = sum(pop(:,1,:,t) .* pop(:,2,:,t)) ./ xi; % This array gives the cooperator proportion for each tag (i.e. it gives, for each tag, the proportion of individuals who are cooperators).
denom = 1 - alpha .* (1-xi) .* (1-F); % This variable (short for denominator) is defined as it is used in multiple calculations below.
Minteract = (F+(1-F).* xi) ./ denom; % This array gives the per-generation probability of obtaining a social interaction.
Mind_helped = (pop(:,2,:,t) .* (phi + (F-phi).* xi) + (F-phi) .* pi + (1-2*F+phi) .* pi .* xi) ./ denom; % This array gives the per-generation probability of receiving help.
Mpop_helped = sum(xi .* pi .* Minteract); % This array gives the population average per-generation probability of receiving help.
Mdeme_helped = ( (Minteract.*pop(:,2,:,t))/N  + Mind_helped/N + (N-2)/N .* ...
               (gamma .* pop(:,2,:,t) + (F-gamma) .* sum(pi.*xi)  + ...
               + (F-gamma)  .* pop(:,2,:,t).*sum(((xi.*pi)./sum(pi.*xi)).*(xi + (1-xi).*alpha.*Minteract)) ...
               + (1-2*F+gamma) .* sum(xi .* pi .* (xi + (1-xi).*alpha.*Minteract))));  % This array gives the deme average per-generation probability of receiving help.
Mind_abandon = ((1-F).*(1-xi).*alpha) ./ denom ; % This array gives the per-generation number of times a partner was abandoned.
Mpop_abandon = sum(xi.*Mind_abandon) ;   % This array gives the population average per-generation number of times a partner was abandoned.
Mdeme_abandon = Mind_abandon/N + ...
                ((1-F).*(1-xi).*alpha.*(1+(Mpop_abandon-xi.*Mind_abandon)./(1-xi)))/N + ...
                ((N-2)/N) .* ...
                ( (F-G).*(1-xi).*alpha.*(1+Mind_abandon) +...
                (F-G).*(1-xi).*alpha.*(1+(Mpop_abandon-xi.*Mind_abandon)./(1-xi)) + ...
                (1-3.*F+2.*G).*sum(xi.*(1-xi).*alpha.*(1+Mind_abandon)) );  % This array gives the deme average per-generation number of times a partner was abandoned.
           
% The below entry (into column 3) gives genotype frequencies after 
% selection (but before recombination and mutation).
pop(:,3,:,t) = pop(:,1,:,t) .* ( 1  +  b .*  Mind_helped - c .* Minteract .* pop(:,2,:,t) - (b-c) .* ((1-m) .* Mdeme_helped  + m .*  Mpop_helped)  - csearch .* (Mind_abandon - (1-m).* Mdeme_abandon - m .* Mpop_abandon ));  
...
% Data generated from this script is saved in the 'Saved_data' folder as 
% 'weak_sel_alpha=...', and was used to generate various figures in the
