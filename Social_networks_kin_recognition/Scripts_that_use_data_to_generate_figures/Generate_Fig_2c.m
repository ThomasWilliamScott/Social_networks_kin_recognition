...
linkageDis(1) =  - sum( sum(pop(:,1,:,1),1) .* (pop(2,1,:,1) ./ sum(pop(:,1,:,1),1) - helperFreq(1)) .* (sum(pop(:,1,:,1),1) - sum(sum(pop(:,1,:,1),1).^2,3) ) ,3); % This is linkage disequilibrium initially.
...
Mpop_helped = sum(xi .* pi .* Minteract);
...
Mdeme_abandon = Mind_abandon/N + ...
                ...
                (1-3.*F+2.*G).*sum(xi.*(1-xi).*alpha.*(1+Mind_abandon)) );
 
            
pavg = sum(pi.*xi); % This gives the population frequency of the cooperation allele.
...
RtagList = ((xi .* (-2 .* F .* pi + F + (pi-1) .* phi + pi) + F .* pi - pi.*phi + phi ) ./ (xi.*(-F) + xi + F) - pavg) ./ (1-pavg);
...
% disequilibrium.
helperFreq(t+1) = sum(pop(2,1,:,t+1),3)  ; 
numberTags(t+1) = 1/(sum(sum(pop(:,1,:,t+1),1).^2,3)); %1/(sum(sum(pop(:,1,:,t+1)).^2))  ;
linkageDis(t+1) =  - sum( sum(pop(:,1,:,t+1),1) .* (pop(2,1,:,t+1) ./ sum(pop(:,1,:,t+1),1) - helperFreq(t+1)) .* (sum(pop(:,1,:,t+1),1) - sum(sum(pop(:,1,:,t+1),1).^2,3) ) ,3);
...
plot(1:T+1,linkageDis,'Color',[0.4660, 0.6740, 0.1880],'LineWidth',2,'LineStyle','-')
