...
% "Data_Fig_5d_alpha=...", to generate Fig. 5d (Main text). The data was 
% collected by iterating the population genetic model described in the main
% text and in Supp. Info. 4b, whilst assuming the following parameter 
% values: tag = 100 (labelled L_{max} in the text); N=30; T=100000; 
% mutC = 0.001 (labelled mu_Trait in the text); tagSkewIni = 0.9; b = 0.3; 
% c = 0.1; helpini = 0.1; variable alpha within the range 0.5 to 1.

clearvars
close all
clc

% The below array shows the range of alpha values that data was collected
% for.
alphaR = [1 0.9999 0.9997 0.999 0.997 0.995 0.99 0.98 0.975 0.95 0.9 0.8 0.5];
...
load("../Saved_data/Data_Fig_5d_alpha=" + alpha + ".mat")
...
% We have so-far obtained an array 'shadingparam' that gives, for different
% values of alpha (partner recycling probabilities), the relative area of
% parameter space where kin discrimination is stable. This is the Y axis of
% Fig. 5d. But on the X axis, we will not plot alpha. Instead, we will plot
% the probability of social interaction associated with each value of 
% alpha, for a rare tag. Therefore, next, we will derive these social
% interaction probabilities.

tag = 1000000000; % Maximum tag number (L_{max} in main text). We set this to a very large number in order because this corresponds to a very rare tag (tag frequency is 1/tag).
listres=NaN; % This will be populated with results.
int = zeros(1,length(alphaR)); % This empty matrix will be populated with results.
...
int(cur_alpha) = nanmean(listres);  % This records, for each value of alpha, the mean social interaction probability, where the mean is taken across all r & m values where kin recognition is favoured by kin selection (Equation 1 in main text is satisfied).

end

% Therefore, we now have, for each value of alpha in our array 'alphaR', a
% corresponding 'shadingparam' value (relative area where kin 
% discrimination stable) as well as a corresponding 'int' value 
% (probability of social interaction for a very rare tag). We now write a
% few lines of code to plot 'shadingparam' directly against 'int'.

figure
plot(int,shadingparam ,'-o','LineWidth',2,'Color','k')
box off
set(gcf,'color','white')
ylim([0 1])
xlim([0 1])