% This script uses data generated by iterating our population genetic model
% described in the main text and Sup. Info. 4b (weak selection island 
% model). The data was saved in the 'Saved_data' folder, in four parts, as 
% 'Island_weak_sel_alpha=1_mu=0_N=10.mat',
% 'Island_weak_sel_alpha=1_mu=0.001_N=10.mat', 
% 'Island_weak_sel_alpha=1_mu=0_N=30.mat',
% 'Island_weak_sel_alpha=1.mat'. This script loads this data to generate 
% Fig. S12. Specifically, it generates the figure in four parts, a-d.

clearvars
clc
close all

for part = [1 2 3 4] % these four iterations generate, respectively, Supp. Fig. 12a, b, c & d.
    
    if part == 1
        load ../Saved_data/Island_weak_sel_alpha=1_mu=0_N=10.mat

    elseif part==2
        load ../Saved_data/Island_weak_sel_alpha=1_mu=0.001_N=10.mat

    elseif part==3
        load ../Saved_data/Island_weak_sel_alpha=1_mu=0_N=30.mat

    elseif part==4
        load ../Saved_data/Island_weak_sel_alpha=1.mat % This file stores data for the N=30, mu=0.001 case.
...
% First, we generate the upper graph, which plots the number of tags
% maintained at equilibrium (1/avgtagfreq), for different values of m & r.
subplot(4,1,1) 
imagesc(mR,rR,1./avgtagfreq)
set(gca,'YDir','normal')
...
% The following 13 lines of code use the resrel matrix to plot a line
% demarcating the region of parameter space where kin discrimination is
% favoured by Hamilton's Rule (Equation 1 in main text satisfied).
hold on
X = mR;
...
% The following four lines of code are for formatting.
colormap(gca, summer)
%colorbar
caxis([1 tag]);
%colorbar('Ticks',[1,1+(tag-1)/2,tag])

% Next, we generate the lower graph, which plots the frequency of the
% cooperation allele at equilibrium (resalt), for different values of m & 
% r.
subplot(4,1,2) 
imagesc(mR,rR,resalt)
set(gca,'YDir','normal')
axis([min(mR) max(mR) min(rR) max(rR)])
yspace=min(rR):max(rR)/5:max(rR);
yticks([yspace]);
...
% The following 3 lines of code plot the line demarcating the region of 
% parameter space where kin discrimination is favoured by Hamilton's Rule 
% (Equation 1 in main text satisfied).
hold on
plot(X,Y,'color','k','Linestyle','-','LineWidth',1)
hold off
