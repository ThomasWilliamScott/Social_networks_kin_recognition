...
% Info. 3c. It plots the region of parameter space where kin discrimination
...
% Fig S7a and the second is Fig S7b.
...
for part = [1  2]
    if part==1
        load ../Saved_data/strong_sel_b=3c.mat
        
    elseif part==2
        load ../Saved_data/strong_sel_b=9c.mat
    end
    
tag=L;  % This step is neccessary because our 'tag' parameter was saved as 'L' in the data files, so we need to equate these alternative names for the same variable (denoted as L_{max} in the main text).
mutC=TraitMu; % This step is neccessary because our 'mutC' parameter was saved as 'TraitMu' in the data files, so we need to equate these alternative names for the same variable.
...
% gamma, phi and F are different coalescence probabilities.
