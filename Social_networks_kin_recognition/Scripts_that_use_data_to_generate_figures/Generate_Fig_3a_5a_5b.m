% This script generates Fig. 3a, 5a & 5b (the "igloo" plots). It generates
% these plots by loading data that has been collected by iterating our
% population genetic model described in the main text and Supp. Info. 4b 
% (weak selection island model), for the following parameter values: 
% tag = 100 (maximum tags), N=30 (deme size), T = 100000 (generations), 
% mutC = 0.001 (trait mutation rate), tagSkewIni = 0.9 (initial frequeny of
% one dominant tag), b = 0.3 (benefit), c = 0.1 (cost), helpini = 0.1 
% (initial helper frequency).
...
for aaaa = [1 2 3]
    ...
    if aaaa == 1
        load ../Saved_data/Island_weak_sel_alpha=1.mat % This loads data for alpha=1, used to generate Fig. 3a.

    elseif aaaa == 2        
        load ../Saved_data/Island_weak_sel_alpha=0.975.mat % This loads data for alpha=0.975, used to generate Fig. 5a.
        
    elseif aaaa == 3        
        load ../Saved_data/Island_weak_sel_alpha=0.mat % This loads data for alpha=0, used to generate Fig. 5b.
...
res = 1./avgtagfreq > 10 & resalt>0.4; % This criterion indicates when (for given m and r values) kin discrimination evolves. It is assumed (arbitrarity) that kin discrimination corresponds to an excess of 10 tags maintained at equilibrium alongside a population cooperator frequency of over 0.4.
[row, column] = find(res == 1);
scatter( mR(column), rR(row),[],[0.5 0.5 0.5],'filled') % This generates a scatter plot, where each point corresponds to parameter (m & r) combinations where kin discrimination evolves.
box on
...
% onto the scatter plots, which show when kin discrimination is stable.
X = mR2;
